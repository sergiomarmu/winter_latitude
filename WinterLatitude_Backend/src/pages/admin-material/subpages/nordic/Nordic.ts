 ...
 */
@Component({
  selector: 'page-Material-SkiNordico',
  templateUrl: 'Nordic.html'
})

/**
 * Clase Nordic, componente de Nordic el cual nos permitira mostrar, añadir, actualizar y borrar
 *toda la información de los precios y stocks de Esquí Nórdico, de la base de datos, Firebase de nuestra aplicación
 */
export class Nordic {
  ...
  /**
   * Constructor de la clase Nordic
   */
  constructor(public navCtrl: NavController,
    ...
    /**
     * Extraemos la información de los materiales de Esquí Nórdico de la BD de FireBase y la guardamos en la variable estacion
     */
    this.material = this.db.list('Material/EsquíNórdico/Estacion/');
  ...
  createMaterial(){
    let newUserModal = this.alertController.create({
      title: "Nuevo Precio/Stock",
      message: "Agrega aquí un nuevo Precio y Stock",
      inputs: [
        {
          name: "name",
          placeholder: "Nombre"
      ...
      buttons:[
        ...
        },
        {
          text: "Guardar",
          handler: data => {
            firebase.database().ref('Material/EsquíNórdico/Estacion/' + data.name).set({
              Precio: data.Precio,
              Stock: data.Stock,
              name: data.name
            });
