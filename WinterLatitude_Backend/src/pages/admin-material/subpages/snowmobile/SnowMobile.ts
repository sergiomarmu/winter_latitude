 ...
 */
@Component({
  selector: 'page-Material-SnowMobile',
  templateUrl: 'SnowMobile.html'
})

/**
 * Clase SnowMobile, componente de SnowMobile el cual nos permitira mostrar, añadir, actualizar y borrar
 * toda la información de los precios y stocks de SnowMobile, de la base de datos, Firebase de nuestra aplicación
 */
export class SnowMobile {
  ...
  constructor(public navCtrl: NavController,
    ...
    /**
     * Extraemos la información de los materiales de SnowMobile de la BD de FireBase y la guardamos en la variable estacion
     */
    this.material = this.db.list('Material/MotosNieve/Estacion/');
    ...
    let newUserModal = this.alertController.create({
      ...
      buttons:[
        ...
        },
        {
          text: "Guardar",
          handler: data => {
            firebase.database().ref('Material/MotosNieve/Estacion/' + data.name).set({
              Precio: data.Precio,
              Stock: data.Stock,
              name: data.name
            });
