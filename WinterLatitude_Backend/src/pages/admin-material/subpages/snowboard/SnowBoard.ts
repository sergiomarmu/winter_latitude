 ...
 */
@Component({
  selector: 'page-Material-SnowBoard',
  templateUrl: 'SnowBoard.html'
})

/**
 * Clase SnowBoard, componente de SnowBoard el cual nos permitira mostrar, añadir, actualizar y borrar
 * toda la información de los precios y stocks de SnowBoard, de la base de datos, Firebase de nuestra aplicación
 */
export class SnowBoard {
  ...
  constructor(public navCtrl: NavController,
    ...
    /**
     * Extraemos la información de los materiales de SnowBoard de la BD de FireBase y la guardamos en la variable estacion
     */
    this.material = this.db.list('Material/SnowBoard/Estacion/');
    ...
    let newUserModal = this.alertController.create({
      ...
      buttons:[
        ...
        },
        {
          text: "Guardar",
          handler: data => {
            firebase.database().ref('Material/SnowBoard/Estacion/' + data.name).set({
              Precio: data.Precio,
              Stock: data.Stock,
              name: data.name
            });
